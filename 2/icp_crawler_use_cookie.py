...
from util import Logger, pad_url, uni_format, concat, is_valid_web, get_file_name, get_cookies_by_resp
import time
...
WebInfo = namedtuple("WebInfo", ["url", "icp", "title", "keywords", "description", "name", "cookies"])
...
class Crawler(Thread):
    ...
    def _request(self, url, cookies=None) -> Response:
        # DONE 处理状态码，超时、重查次数
        """
        :param url:
        :return req:
    ...
    def _extract(self, req: Response, url: str) -> WebInfo:
        ...
        try:
            ...
            description = ""
        try:
            cookies = get_cookies_by_resp(req)
        except Exception as e:
            LOG.debug("get cookies failed, exceptions: {}".format(e))
            cookies = ""
        ...
        web_info = WebInfo(url, icp, title, keywords, description, names, cookies)
        self.done_num += 1
        return web_info
