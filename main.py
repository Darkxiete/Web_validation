...
if __name__ == '__main__':
    ...
    if '1' in steps:
        print("Step 1 解密")
        ex_cmd("python 1/decrypt.py -p 1/datas")
    ...
    if '3' in steps:
        ...
        for file in glob("datas/*_ret_hosts.csv"):
            ...
            if not exists(file_dir):
                mkdir(file_dir)
        ex_cmd("python crawler.py -p datas/total_hosts.csv")
        chdir("..")

    if '4' in steps:
        print("Step 4 分发图片")
        last_pics_path = get_last_list("3/pics")
        ex_cmd("python move_pics_by_csv.py -p1 3\datas -p2 3\pics\{}".format(last_pics_path))